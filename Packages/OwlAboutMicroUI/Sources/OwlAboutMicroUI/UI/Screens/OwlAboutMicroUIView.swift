...
struct OwlAboutMicroUIView: View {

    @State private var viewModel: OwlAboutMicroUIViewModel
    @Injected(\.owlaboutNavigationCoordinator) private var coordinator
    @State private var path = NavigationPath()

    init(viewModel: OwlAboutMicroUIViewModel) {
        ...
        NavigationStack(path: $path) {
            Group {
                if viewModel.isLoading {
                    OwlsLoadingView("Loading owlabout…")
            ...
            .navigationTitle("OwlAbout")
            // MARK: Navigation — push to detail screen
            .navigationDestination(for: OwlAboutMicroUIRouter.self) { route in
            ...
            .sheet(isPresented: $viewModel.isCreateSheetPresented) {
                OwlAboutCreateSheet(viewModel: viewModel)
        ...
        List {
            ForEach(viewModel.items) { item in
                Button {
                    ...
                    path.append(OwlAboutMicroUIRouter.detail(item))
                } label: {
