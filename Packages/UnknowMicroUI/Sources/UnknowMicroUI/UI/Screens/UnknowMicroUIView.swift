...
struct UnknowMicroUIView: View {

    @State private var viewModel: UnknowMicroUIViewModel
    @Injected(\.unknowNavigationCoordinator) private var coordinator
    @State private var path = NavigationPath()

    init(viewModel: UnknowMicroUIViewModel) {
        ...
        NavigationStack(path: $path) {
            Group {
                if viewModel.isLoading {
                    OwlsLoadingView("Loading unknow…")
            ...
            .navigationTitle("Unknow")
            // MARK: Navigation — push to detail screen
            .navigationDestination(for: UnknowMicroUIRouter.self) { route in
            ...
            .sheet(isPresented: $viewModel.isCreateSheetPresented) {
                UnknowCreateSheet(viewModel: viewModel)
        ...
        List {
            ForEach(viewModel.items) { item in
                Button {
                    ...
                    path.append(UnknowMicroUIRouter.detail(item))
                } label: {
