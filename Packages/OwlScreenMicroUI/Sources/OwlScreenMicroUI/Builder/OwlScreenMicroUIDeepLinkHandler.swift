...
//
// Handles URLs like: owlsapp://owlscreen/detail/123
//
// Registered in Config.swift via:
//   OwlsDeepLinkRouter.shared.register(OwlScreenMicroUIDeepLinkHandler())

struct OwlScreenMicroUIDeepLinkHandler: OwlsDeepLinkHandler {

    var supportedModules: [String] { ["owlscreen"] }

    func handle(_ deepLink: OwlsDeepLink) -> Bool {
        let coordinator = Container.shared.owlscreenNavigationCoordinator()

        // Parse the path: "owlscreen/detail/123"
